...
export default async function FollowersPage({
  ...
  // 2. Fetch daftar profil orang-orang yang mem-follow targetProfile ini
  // Trik !inner join: Ambil semua data profiles yang ID-nya ada di kolom follower_id tabel user_followers
  const { data: followers, error } = await supabase
    .from("profiles")
    .select("id, slug, first_name, last_name, avatar_url, role, user_followers!user_followers_follower_id_fkey!inner(user_id)")
    .eq("user_followers.user_id", targetProfile.id);

  if (error) {
    console.error("Error fetching followers:", error.message);
  }

  const followersList = (followers as unknown as FollowerProfile[]) || [];
          ...
          {followersList.map((follower) => (
            ...
            >
              <div className="w-20 h-20 sm:w-24 sm:h-24 rounded-full overflow-hidden border-2 border-gray-100 group-hover:border-[#00a896] transition-colors mb-3 shadow-sm">
                {follower.avatar_url ? (
                  ...
                  <div className="w-full h-full bg-slate-100 flex items-center justify-center text-slate-400 font-bold text-xl uppercase">
                    {follower.first_name.substring(0, 2)}
                  </div>
                )}
              </div>
              ...
              <div className="text-center w-full px-1">
                ...
                {/* Badge khusus admin jika yang follow ternyata admin */}
                {follower.role === "admin" ? (
